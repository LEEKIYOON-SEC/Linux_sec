# secchk — 망분리 Linux 서버 침해흔적 점검 스크립트

매일 1회 cron으로 자동 실행되어 호스트 내부의 침해 흔적을 찾아내고, 결과를 로컬에 저장하면 운영자가 SSH로 회수하는 단일 쉘 스크립트입니다. 외부 통신 없이 동작하므로 망분리 환경에서 그대로 사용할 수 있습니다.
...
방화벽·WAF·IDS 같은 네트워크 장비만으로는 "이 서버 내부가 실제로 침해됐는가?"를 알기 어렵습니다. 본 도구는 각 서버에서 자체적으로 침해의 흔적을 찾는 16종 점검을 매일 1회 수행하고, 결과를 운영자가 매일 1분 안에 확인할 수 있는 형태로 정리해줍니다.
...
본 도구는 **침해를 발견하는 도구**이지 **차단하는 도구가 아닙니다**. 발견된 신호를 보고 다음 행동(보존·격리·포렌식)을 결정하는 것은 운영자의 SOP입니다.
...
TRUSTED_DNS="10.10.0.1 10.10.0.2"             # 사내 DNS IP
```
...
→ `STATUS:CLEAN` 또는 `STATUS:CLEAN WARMUP_PERIOD`가 나오면 성공입니다.
...
`STATUS:CLEAN`이면 오늘은 별다른 일이 없다는 뜻입니다.
...
`STATUS:CLEAN`이면 끝. `STATUS:ALERT`이면 상세 확인으로 넘어갑니다.
...
# ─── 신뢰 외부 DNS (이외의 외부 IP DNS는 HIGH 알람) ─────
TRUSTED_DNS="10.10.0.1 10.10.0.2"
...
ClamAV는 알려진 악성코드 시그니처와 디스크 파일을 비교하는 안티바이러스입니다. 시그니처 갱신을 위해 외부 인터넷이 필요하므로, 망분리 환경에서는 USB로 정기 반입해야 합니다.
...
분기 1회 이상 (가능하면 월 1회). 시그니처 파일이 30일을 넘으면 본 도구가 자동으로 `MEDIUM stale_signature` 경고를 띄웁니다.
...
**왜 봐야 하나요?**
본 도구의 다른 모듈은 "행위 기반"(어제와 다른 변화) 또는 "패턴 기반"(웹쉘 정규식) 검사인데, 이미 알려진 트로이안/웜/백도어 변종은 시그니처 매칭이 가장 빠르고 정확합니다. 다른 검사를 보완하는 레이어입니다.
...
본 도구는 어제 vs 오늘 비교를 핵심으로 합니다. 그런데 콜드 영역은 요일별로 한 번씩만 점검되므로, 첫 비교가 가능한 시점이 **가동 후 8일째**입니다. 그 전에 알람을 띄우면 단순히 "처음 본 데이터"가 모두 신규로 잡혀 거짓양성이 폭증합니다.
...
본 도구의 범위 밖. 사내 보안팀 또는 외부 전문 인력에 인계.

### 자동 자가복구를 하지 않는 이유

본 도구는 발견한 침해 흔적을 **자동으로 제거하지 않습니다**. 다음 이유 때문입니다.
...
### Q3. 첫 도입 시 false positive가 너무 많아요
...
- 보통 1주차에 익숙해지면 false positive 패턴이 안정화됨
...
없습니다. 모든 CSS는 인라인이며 JavaScript도 없습니다. 망분리 환경에서 그대로 열립니다.
...
| 관리자 권한 공격자는 점검 결과 자체를 위조할 수 있음 | `chattr +i`로 일부 지연 효과. 완전 보호는 결과를 별도 시스템으로 외부 전달해야 가능 (본 도구 범위 밖) |
...
| 신종/맞춤 악성코드는 시그니처가 없어 못 잡음 | 본 도구의 행위 기반 검사(13/15/14)가 보강. 시그니처 의존 영역은 ClamAV 1개 모듈로 한정 |
...
| **WARMUP_PERIOD** | 본 도구의 가동 초기 안정화 기간 (8일). 어제 vs 오늘 비교가 의미 있어지기 전까지 일부 검사를 격하해 거짓양성을 방지. |
| **핫스팟 / 콜드 영역** | 디스크 점검 대상의 분류. 핫스팟(공격 표적, 변동 잦음)은 매일, 콜드 영역(시스템 디렉토리)은 요일별 1/7씩. |
| **chattr +i** | ext 파일시스템의 immutable 속성. 설정된 파일은 root 권한이어도 삭제/수정이 거부됩니다. 본 도구는 결과 파일을 보호하는 데 사용. |
