...
- [15. 이 스크립트의 한계와 보완 방법](#15-이-스크립트의-한계와-보완-방법)
...
방화벽·WAF·IDS 같은 네트워크 장비만으로는 "이 서버 내부가 실제로 침해됐는가?"를 알기 어렵습니다. 이 스크립트는 각 서버에서 자체적으로 침해의 흔적을 찾는 16종 점검을 매일 1회 수행하고, 결과를 운영자가 매일 1분 안에 확인할 수 있는 형태로 정리해줍니다.
...
이 스크립트는 **침해 흔적을 찾는 도구**이지 **차단하는 도구가 아닙니다**. 발견된 신호를 보고 다음 행동(보존·격리·포렌식)을 결정하는 것은 운영자의 SOP입니다.
...
매일 03:30 cron
    │
    ▼
secchk.sh --mode daily   (16개 점검 모듈 순차 실행)
    │
    ▼
output/YYYYMMDD/  +  output/latest 심볼릭링크 갱신
```
...
| 그룹 | 모듈 |
|---|---|
| 계정·접근 | 10 계정 / 11 로그인 이력 / 16 SSH 인증 |
| 네트워크 | 12 네트워크 상태 / 21 네트워크 설정 |
| 프로세스·파일 | 13 프로세스 / 14 파일 이상 / 15 지속성 |
| 시스템 무결성 | 18 로그 변조 / 19 커널 모듈 / 20 패키지 무결성 |
| 응용 영역 | 17 웹쉘 / 22 메일 큐 / 23 컨테이너 |
| 안티바이러스 | 24 ClamAV / 25 rkhunter(옵션) |
...
추가로 `output/` 루트에는 `latest` 심볼릭링크와 일자별 누적인 `SUMMARY_INDEX.txt`가 자동 생성됩니다.

**핵심 동작 원리 두 가지**

1. **어제 vs 오늘 자가 diff** — "갑자기 생긴 것"을 탐지. 어제 없던 SSH 키, 어제 없던 LISTEN 포트, 어제 없던 SUID 파일 등.
...
> 위 이미지는 실제 `secchk.sh` 가 생성한 `output/latest/report.html` 을 그대로 캡처한 것입니다. 운영자가 `scp` 로 받아 브라우저로 열면 같은 화면을 봅니다.
...
분기 1회 이상 (가능하면 월 1회). 시그니처 파일이 30일을 넘으면 secchk 가 자동으로 `MEDIUM stale_signature` 경고를 띄웁니다.
...
**왜 봐야 하나요?**
공격자가 서버를 스팸 발송 봇으로 활용하면 메일 큐에 발송 대기 메일이 쌓입니다. 침해된 서버에서 평소와 다른 메일 트래픽이 발생하는 가장 흔한 신호입니다.
...
**왜 봐야 하나요?**
이 스크립트의 다른 모듈은 "행위 기반"(어제와 다른 변화) 또는 "패턴 기반"(웹쉘 정규식) 검사인데, 이미 알려진 트로이안/웜/백도어 변종은 시그니처 매칭이 가장 빠르고 정확합니다. 다른 검사를 보완하는 레이어입니다.
...
이 스크립트는 어제 vs 오늘 비교를 핵심으로 합니다. 그런데 콜드 영역은 요일별로 한 번씩만 점검되므로, 첫 비교가 가능한 시점이 **가동 후 8일째**입니다. 그 전에 알람을 띄우면 단순히 "처음 본 데이터"가 모두 신규로 잡혀 거짓양성이 폭증합니다.
...
> **STATUS:WARN(MEDIUM만 발견)일 때**: 즉시 격리 단계까지 갈 일은 드물지만, 어떤 검사가 잡혔는지 보고 정상 변경인지 확인합니다. `result.json`에서 MEDIUM 항목만 추출해 운영팀이 인지하는 변경인지 확인 → 모르는 변경이면 아래 SOP에 준해서 대응. 익숙해진 반복 MEDIUM(예: 컨테이너 sudoers의 운영자 NOPASSWD 라인)은 무시해도 좋습니다. 다만 같은 MEDIUM이 매일 계속 보이는 게 아니라 새로 추가된 MEDIUM이라면 한 번 확인하는 게 안전합니다.
...
secchk 의 범위 밖. 사내 보안팀 또는 외부 전문 인력에 인계.

### 자동 자가복구를 하지 않는 이유

이 스크립트는 발견한 침해 흔적을 **자동으로 제거하지 않습니다**. 다음 이유 때문입니다.
...
## 15. 이 스크립트의 한계와 보완 방법

| 한계 | 보완 방법 (간략) |
|---|---|
| 매일 1회 점검이라 그 사이 발생했다가 사라진 짧은 공격은 놓침 | cron 주기를 시간 단위로 좁히거나, 점검 시각에 무작위 지연 (`sleep $((RANDOM%600))`)으로 공격자의 시각 예측 어렵게 |
| 관리자 권한 공격자는 점검 결과 자체를 위조할 수 있음 | `chattr +i`로 일부 지연 효과. 완전 보호는 결과를 별도 시스템으로 외부 전달해야 가능 (secchk 범위 밖) |
...
| 신종/맞춤 악성코드는 시그니처가 없어 못 잡음 | 이 스크립트의 행위 기반 검사(13/15/14)가 보강. 시그니처 의존 영역은 ClamAV 1개 모듈로 한정 |

이 스크립트는 위 한계를 안고 있는 **정기 점검 도구**입니다. 한계를 인지하고 운영하면 충분히 실용적인 가시성을 제공합니다.
...
| **EICAR** | 안티바이러스 테스트용 표준 더미 파일. 실제 악성 기능은 없지만 모든 AV가 이를 탐지하도록 약속되어 있어 검증에 사용. ClamAV 도입 후 24 모듈이 정상 동작하는지 확인하려면 `/tmp` 에 EICAR 문자열을 한 줄 저장하고 점검을 돌리면 됩니다. |
| **WARMUP_PERIOD** | secchk 의 가동 초기 안정화 기간 (8일). 어제 vs 오늘 비교가 의미 있어지기 전까지 일부 검사를 격하해 거짓양성을 방지. |
| **핫스팟 / 콜드 영역** | 디스크 점검 대상의 분류. 핫스팟(공격 표적, 변동 잦음)은 매일, 콜드 영역(시스템 디렉토리)은 요일별 1/7씩. |
| **chattr +i** | ext 파일시스템의 immutable 속성. 설정된 파일은 root 권한이어도 삭제/수정이 거부됩니다. secchk 는 결과 파일을 보호하는 데 사용. |
